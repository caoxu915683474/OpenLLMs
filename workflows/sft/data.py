...
class DatasetHelper:
    """ DatasetHelper """
    def __init__(self, args: "DataArguments", tokenizer: "PreTrainedTokenizer") -> None:
        """ __init__ """
        self.args = args
        ...
        if self.args.do_train:
            if self.args.val_size > 1e-6:  # Split the dataset
                if self.args.streaming:
                    ...
                    self.dataset = self.dataset.shuffle(buffer_size=self.args.buffer_size, seed=self.args.seed)
                    ...
                    self.dataset = self.dataset.train_test_split(test_size=val_size, seed=self.args.seed)
