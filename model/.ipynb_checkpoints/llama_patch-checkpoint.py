...
def llama_torch_attn_forward(self: "LlamaAttention",
    ...
    query_states = query_states.view(bsz, 
                                     q_len, 
                                     self.num_heads, 
                                     self.head_dim).transpose(1, 2)
    key_states = key_states.view(bsz, 
                                 q_len, 
                                 self.num_key_value_heads, 
                                 self.head_dim).transpose(1, 2)
    value_states = value_states.view(bsz, 
                                     q_len, 
                                     self.num_key_value_heads, 
                                     self.head_dim).transpose(1, 2)
    ...
    if getattr(self.config, "group_size_ratio", None) and self.training:  # shift
        ...
        def shift(state: torch.Tensor) -> torch.Tensor:
            ...
            state = torch.cat((state[:, :, : self.num_heads // 2], 
    ...
    if getattr(self.config, "group_size_ratio", None) and self.training:  # shift back
        ...
        attn_output = torch.cat((attn_output[:, :, : self.num_heads // 2], 
...
def llama_flash_attn_forward(self: "LlamaFlashAttention2",
                             hidden_states: torch.Tensor,
                             attention_mask: Optional[torch.Tensor] = None,
                             position_ids: Optional[torch.LongTensor] = None,
    ...
    if getattr(self.config, "group_size_ratio", None) and self.training:  # shift
        ...
        if attention_mask is not None:
            attention_mask = attention_mask[:, :, :groupsz, :groupsz].repeat(num_groups, 1, 1, 1)
    attn_output: torch.Tensor = self._flash_attention_forward(query_states, 
                                                              key_states, 
                                                              value_states, 
                                                              attention_mask, 
                                                              q_len, 
