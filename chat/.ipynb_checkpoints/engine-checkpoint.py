...
class VLLMEngine(BaseEngine):
    ...
    def __init__(self, 
        ...
        engine_args = AsyncEngineArgs(model=model_args.path, 
    ...
    async def generate(self, 
        ...
        generating_args.update(dict(temperature=temperature or generating_args["temperature"],
                                    top_p=top_p or generating_args["top_p"],
                                    top_k=top_k or generating_args["top_k"],
                                    num_return_sequences=num_return_sequences or 1,
                                    repetition_penalty=repetition_penalty or \
                                        generating_args["repetition_penalty"]))
        if max_length:
            generating_args["max_new_tokens"] = max_length - prompt_length
        if max_new_tokens:
            generating_args["max_new_tokens"] = max_new_tokens
        sampling_params = SamplingParams(n=generating_args["num_return_sequences"],
        ...
        result_generator = self.model.generate(prompt=None, 
    ...
    async def chat(self,
        ...
        for output in final_output.outputs:
            results.append(Response(response_text=output.text,
    ...
    async def stream_chat(self,
        ...
        """ stream_chat """
        generated_text = ""
        generator = await self.generate(messages, system, tools, **input_kwargs)
